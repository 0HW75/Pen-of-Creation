...
import ChapterAppearanceModal from './SettingManagement/Common/ChapterAppearanceModal';
...
const CharacterCard = ({
  ...
  const loadChapterCount = async () => {
    try {
      const res = await chapterAppearanceApi.getEntityAppearances('character', character.id);
            ...
            <div className="character-card-cover">
              ...
              {chapterCount > 0 && (
                <Tooltip title={`在 ${chapterCount} 个章节中出现`}>
                  <div className="character-chapter-badge">
                    <Badge count={chapterCount} size="small" style={{ backgroundColor: '#1890ff' }}>
