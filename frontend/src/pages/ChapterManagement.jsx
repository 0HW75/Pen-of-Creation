...
const ChapterManagement = ({ projectId, onChapterSelect, onChaptersChange }) => {
  ...
  const ChapterForm = () => {
    ...
    return (
      ...
      >
        ...
        >
          <Select
            placeholder="请选择或输入卷/分组"
            allowClear
            showSearch
            mode="tags"
            maxCount={1}
            options={existingVolumes.map(v => ({ label: v, value: v }))}
          />
        </Form.Item>
        ...
        >
          <InputNumber min={0} placeholder="请输入排序索引" style={{ width: '100%' }} />
        </Form.Item>
  ...
  const handleSubmit = async (values) => {
    ...
    // 处理 volume 字段 - 可能是数组或字符串
    let volumeValue = '';
    if (values.volume) {
      if (Array.isArray(values.volume)) {
        volumeValue = values.volume[0] || '';
      } else {
        volumeValue = values.volume;
      }
    }
    // 替换 values 中的 volume 为处理后的值
    ...
    try {
      if (isEditing) {
        ...
        await chapterApi.updateChapter(currentChapter.id, processedValues);
        ...
        const chapterData = {
          ...
          volume: processedValues.volume,
          content: processedValues.content || '',
          status: processedValues.status || '未写',
          type: processedValues.type || '普通',
          order_index: chapters.length
        };
