...
// 导入AI创建世界向导
import CreateWorldWizard from '../../components/CreateWorldWizard';
...
const WorldManagementPanel = ({ onSelectWorld }) => {
  ...
  // AI创建世界相关状态
  const [createModeModalVisible, setCreateModeModalVisible] = useState(false);
  ...
  const showModal = (world = null) => {
    if (world) {
      ...
      // 新建世界时显示创建方式选择
      setCreateModeModalVisible(true);
    }
  };
  
  // 选择手动创建
  const handleManualCreate = () => {
    setCreateModeModalVisible(false);
  ...
  // AI向导完成回调
  const handleAIWizardComplete = (worldId) => {
    setAiWizardVisible(false);
    fetchWorlds();
    message.success('世界创建成功！');
    // 可选：自动选中新创建的世界
    const newWorld = worlds.find(w => w.id === worldId);
    if (newWorld) {
  ...
  return (
    <div className="world-management">
      ...
      <Modal
        title="创建世界"
        open={createModeModalVisible}
        onCancel={() => setCreateModeModalVisible(false)}
        footer={null}
        width={600}
        destroyOnHidden
      >
        <div style={{ textAlign: 'center', padding: '20px 0' }}>
          <h3 style={{ marginBottom: '30px' }}>选择创建方式</h3>
          <Row gutter={24} justify="center">
            <Col span={10}>
              ...
              >
                ...
                <h4>手动创建</h4>
                <p style={{ color: '#666', fontSize: '14px' }}>从零开始<br/>手动填写世界信息</p>
              </Card>
            </Col>
            <Col span={10}>
              <Card
                ...
                style={{ textAlign: 'center', height: '200px', borderColor: '#722ed1' }}
                bodyStyle={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center', height: '100%' }}
              >
                <RobotOutlined style={{ fontSize: '48px', color: '#722ed1', marginBottom: '16px' }} />
                <h4 style={{ color: '#722ed1' }}>AI从蓝图生成</h4>
                <p style={{ color: '#666', fontSize: '14px' }}>从故事蓝图智能<br/>生成世界观设定</p>
              </Card>
            </Col>
          </Row>
        </div>
      ...
      <CreateWorldWizard
        ...
        onCancel={() => setAiWizardVisible(false)}
