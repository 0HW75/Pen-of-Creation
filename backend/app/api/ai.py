...
@api_bp.route('/ai/generate-project-proposal', methods=['POST'])
def generate_project_proposal():
    ...
    if not idea:
        return jsonify({'error': '缺少创意想法'}), 400

    # 构建系统提示
    system_prompt = '''你是一位专业的小说创作顾问，擅长根据用户的创意想法生成完整的项目提案。
请根据用户的创意，生成包含以下字段的JSON格式项目信息：
- title: 作品名（有吸引力的书名）
- pen_name: 笔名（适合该风格的作者名）
- genre: 作品类型（如：玄幻、言情、悬疑、科幻、奇幻、历史、都市、武侠、仙侠等）
- target_audience: 目标读者（如：青少年、成人、儿童、特定群体）
- core_theme: 核心主题（作品要表达的核心思想或价值观，100-200字）
- synopsis: 一句话梗概（用一句话概括故事核心内容）
- writing_style: 创作风格（如：华丽、简洁、悬疑、温馨、幽默、严肃、浪漫、写实等）
- reference_works: 参考作品（类似风格的知名作品，可选）
- suggestion: AI建议（对该创意的创作建议和发展方向，100-150字）

请确保返回的是合法的JSON格式，不要包含任何其他说明文字。'''

    # 构建用户提示
    user_prompt = f'''请根据以下创意想法生成项目提案：

创意想法：{idea}

请直接返回JSON格式的项目信息，不要包含任何其他说明。'''
    ...
    logger.info(f'开始生成项目提案，idea: {idea[:100]}..., provider: {provider}, temperature: {temperature}')
    ...
    try:
        ...
        # 验证必要字段
        required_fields = ['title', 'pen_name', 'genre', 'target_audience', 'core_theme', 'synopsis', 'writing_style']
        for field in required_fields:
            if field not in proposal:
                proposal[field] = ''

        # 确保可选字段存在
        if 'reference_works' not in proposal:
            proposal['reference_works'] = ''
        if 'suggestion' not in proposal:
            proposal['suggestion'] = '祝您创作顺利！'

        return jsonify({'success': True, 'proposal': proposal, 'provider': result.get("provider")})

    except json.JSONDecodeError as e:
        logger.error(f'解析项目提案JSON失败: {str(e)}, content: {content[:200]}')
        ...
        logger.error(f'生成项目提案时发生错误: {str(e)}')
        return jsonify({'error': f'生成失败: {str(e)}'}), 500
