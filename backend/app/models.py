...
class Volume(db.Model):
    ...
    character_development = db.Column(db.Text, default='')
    key_events = db.Column(db.Text, default='[]')  # JSON格式存储
    ...
    def to_dict(self):
        ...
        key_events_list = []
        try:
            if hasattr(self, 'key_events') and self.key_events:
                try:
                    key_events_list = json.loads(self.key_events)
                    if not isinstance(key_events_list, list):
                        key_events_list = []
                except (json.JSONDecodeError, TypeError):
                    # 如果解析失败，尝试作为字符串处理
                    if isinstance(self.key_events, str):
                        key_events_list = [self.key_events] if self.key_events else []
        except Exception as e:
            print(f"解析key_events失败: {e}")
            key_events_list = []
        ...
        return {
            ...
            'core_conflict': self.core_conflict if hasattr(self, 'core_conflict') else '',
            'character_development': self.character_development if hasattr(self, 'character_development') else '',
            'key_events': key_events_list,
            'chapter_count': self.chapter_count if hasattr(self, 'chapter_count') else 0,
...
class AIGenerationVersion(db.Model):
    """AI生成版本管理表 - 存储AI生成的各个版本内容"""
    __tablename__ = 'ai_generation_versions'
    
    id = db.Column(db.Integer, primary_key=True, autoincrement=True)
    project_id = db.Column(db.Integer, db.ForeignKey('project.id'), nullable=False)
    entity_type = db.Column(db.String(50), nullable=False)  # outline/volume/chapter/content
    entity_id = db.Column(db.Integer, nullable=False)  # 关联实体的ID
    version_number = db.Column(db.Integer, default=1)  # 版本号
    version_name = db.Column(db.String(255), default='')  # 版本名称（如"AI生成-1"）
    content = db.Column(db.Text, default='')  # 生成的内容
    prompt = db.Column(db.Text, default='')  # 使用的提示词
    provider = db.Column(db.String(50), default='')  # AI服务提供商
    temperature = db.Column(db.Float, default=0.7)  # 温度参数
    is_current = db.Column(db.Boolean, default=False)  # 是否为当前使用版本
...
class AIGenerationCheckpoint(db.Model):
    ...
    stage = db.Column(db.String(50), nullable=False)  # extraction/generation
    checkpoint_type = db.Column(db.String(50), nullable=True)  # outline/volume/chapter/element
...
class ConceptMergeRecord(db.Model):
    """概念合并记录表 - 存储重复概念合并的历史记录"""
    __tablename__ = 'concept_merge_records'
    
    id = db.Column(db.Integer, primary_key=True, autoincrement=True)
    session_id = db.Column(db.String(64), nullable=False, index=True)
    concept_type = db.Column(db.String(50), nullable=False)
    source_concepts = db.Column(db.Text, nullable=True)  # JSON数组，包含章节来源
    merged_concept = db.Column(db.Text, nullable=True)  # JSON，按章节组织
    ...
    def to_dict(self):
        return {
            ...
            'merged_concept': self.merged_concept,
...
class EntityChapterAppearance(db.Model):
    """实体章节出现索引表 - 记录世界观实体在章节中的出现"""
    __tablename__ = 'entity_chapter_appearances'
    id = db.Column(db.Integer, primary_key=True, autoincrement=True)
    project_id = db.Column(db.Integer, db.ForeignKey('project.id'), nullable=False)
